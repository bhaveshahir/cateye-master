...
//            $arr_row[$col->name] = trim(stripslashes($row[$col->name]));
            $arr_row[$col->name] = trim(($row[$col->name]));
            $c++;
        }
...
function mysql_query_custom($query, $con = "") {
    ...
    return mysql_query($query, $con) or print(mysql_error()."<hr>");
}
...
function insertorupdate($table, $data, $where, $asIs = "", $con) {
    global $dbh;
    $sets = array();
    $sSQL = "select count(*) as counts from `{$table}` where {$where}";
    $result = sql($sSQL, $dbh);
...
function _insert(array $data) {
    ...
    if (is_array($data)) {
        if (!array_key_exists('audit_created_by', $data)) {
            $data['audit_created_by'] = getSession('userid');
        }
...
function email($doseto, $dosefrom, $dosesubject, $dosebody, $dosereplyto, $template, $attachment=null, $log=true) {
    global $include_dir;
    $smtphost = SMTP_HOST;
    ...
    $debugmode = 0;
    $_log_data['`to`'] = $doseto;
    $_log_data['`from`'] = $dosefrom;
    $_log_data['`subject`'] = $dosesubject;
    $_log_data['`body`'] = $dosebody;
    $_log_data['`cc`'] = '';
    $_log_data['`bcc`'] = '';
    $_log_data['`email_template`'] = $template;
    /* if(!isset($_SESSION["dosentmails"])) {
      $_SESSION["dosentmails"] = array();
      }
      if(!in_array(md5($doseto.$dosesubject.$dosebody),$_SESSION["dosentmails"])) { */
    if (file_exists($include_dir.'/includes/mail/class.phpmailer.php')) {
        include_once($include_dir.'/includes/mail/class.smtp.php');
        include_once($include_dir.'/includes/mail/class.phpmailer.php');
        $mail = new PHPMailer();
...
function store_mail_log($_data){
    global $dbh;

    $_data['audit_created_date'] = date('Y-m-d H:i:s');
    $_data['audit_created_by'] = $_SESSION['userid'];
    $_data['audit_ip'] = $_SERVER['REMOTE_ADDR'];

    insert('ms_mail_log', $_data, $dbh);
}
